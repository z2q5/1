...
def apply_theme():
    if st.session_state.theme == "dark":
        ...
        .stSelectbox, .stTextInput { color: white !important; }
        </style>
...
with col2:
    lang = st.selectbox("🌐 " + t("اللغة", "Language"), ["ar", "en"], index=0 if st.session_state.lang == "ar" else 1)
    st.session_state.lang = lang
with col3:
    theme = st.selectbox("🎨 " + t("الثيم", "Theme"), ["light", "dark"], index=0 if st.session_state.theme == "light" else 1)
...
# ===================== واجهة الطالب =====================
if selected.startswith("الطالب") or selected.startswith("Student"):
    st.subheader("👨‍🎓 " + t("تسجيل حالة الحضور", "Mark Attendance"))
    sid = st.text_input(t("رقم الطالب", "Student ID"), key="sid")
    sname = st.text_input(t("اسم الطالب", "Student Name"), key="sname")
    bus = st.selectbox(t("رقم الباص", "Bus Number"), ["1", "2", "3"], key="bus")

    colA, colB = st.columns(2)
    with colA:
        if st.button("✅ " + t("سآتي اليوم", "Coming Today")):
    ...
    with colB:
        if st.button("❌ " + t("لن آتي اليوم", "Not Coming Today")):
...
# ===================== واجهة السائق =====================
elif selected.startswith("السائق") or selected.startswith("Driver"):
    st.subheader("🚍 " + t("عرض حالة الطلاب", "View Bus Students"))
    bus_num = st.selectbox(t("اختر رقم الباص", "Select Bus Number"), ["1", "2", "3"])
...
# ===================== واجهة الإدارة =====================
elif selected.startswith("الإدارة") or selected.startswith("Admin"):
    st.subheader("🏫 " + t("لوحة تحكم الإدارة", "Admin Panel"))
    ap = st.text_input(t("كلمة المرور", "Password"), type="password", key="adminpass")
...
# ===================== واجهة حول / Credits =====================
elif selected.startswith("حول") or selected.startswith("About"):
