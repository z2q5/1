...
def load_data():
    ...
    try:
        ...
        if (DATA_DIR / "settings.json").exists():
            with open(DDATA_DIR / "settings.json", "r", encoding="utf-8") as f:
...
def show_driver_page():
    """صفحة السائق"""
    st.info("🚌 صفحة السائق - قيد التطوير")
    st.write("هذه الصفحة مخصصة لسائقي الباصات لتسجيل الدخول وعرض قوائم الطلاب")

def show_parents_page():
    """صفحة أولياء الأمور"""
    st.info("👨‍👩‍👧 صفحة أولياء الأمور - قيد التطوير")
    st.write("هذه الصفحة مخصصة لأولياء الأمور لمتابعة أبنائهم ومعلومات الباص")

def show_admin_page():
    """صفحة الإدارة"""
    st.info("🏫 صفحة الإدارة - قيد التطوير")
    st.write("هذه الصفحة مخصصة لإدارة النظام والبيانات والإعدادات")

def show_support_page():
    """صفحة الدعم"""
    st.info("🤖 صفحة الدعم - قيد التطوير")
    st.write("هذه الصفحة تحتوي على المساعد الذكي ونظام التواصل مع المطور")

def show_about_page():
    """صفحة حول النظام"""
    st.info("ℹ️ صفحة حول النظام - قيد التطوير")
    st.write("هذه الصفحة تحتوي على معلومات عن النظام والفريق والميزات")
