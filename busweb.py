...
def register_attendance(student, status):
    ...
    # إزالة أي تسجيلات سابقة لنفس اليوم
...
def send_parent_notification(student_id, message_type):
    ...
    show_notification(f"📱 {message}", "info")
...
def context_help():
    ...
    help_messages = {
        "student": "💡 **نصائح للطلاب:** أدخل رقم وزارتك بدقة وسجل حضورك قبل الساعة 8 صباحاً",
        "driver": "💡 **نصائح للسائقين:** تأكد من كلمة المرور قبل الدخول وراجع قائمة الحضور",
        "parents": "💡 **نصائح لأولياء الأمور:** احفظ رقم وزارة ابنك وتابع حالة الباص بانتظام",
        "admin": "💡 **نصائح للإدارة:** احفظ نسخة احتياطية بانتظام وراجع التقارير اليومية",
        "support": "💡 **نصائح للدعم:** استخدم المساعد الذكي للإجابة السريعة على استفساراتك",
        "about": "💡 **حول النظام:** نظام متكامل لإدارة النقل المدرسي بواجهة سهلة الاستخدام"
    }
...
def handle_ai_response(user_message):
    ...
    responses = {
        ...
        "مشكلة في التسجيل": "إذا واجهت مشكلة في التسجيل: 1- تأكد من رقم الوزارة 2- تحقق من اتصال الإنترنت 3- جرب تحديث الصفحة",
        "أريد التواصل مع المطور": "يمكنك التواصل مع المطور عبر البريد الإلكتروني: eyadmustafaali99@gmail.com",
        "default": "شكراً لاستفسارك! يمكنني مساعدتك في: تسجيل الحضور، متابعة الباص، حل المشكلات التقنية."
    }
...
with col1:
    ...
    <div class='metric-card'>
        ...
        <h2 style='color: #10b981;'>{stats['percentage']:.0f}%</h2>
        <p>{stats['coming']}/{stats['total']} طالب</p>
    </div>
    """, unsafe_allow_html=True)
...
for i, (name, page_key) in enumerate(pages):
    with nav_cols[i]:
        ...
        if st.button(name, use_container_width=True, type="primary" if is_active else "secondary", key=f"nav_{page_key}"):
...
if st.session_state.page == "student":
    ...
    with col2:
        st.markdown(f"<h3>{t('stats_title')}</h3>", unsafe_allow_html=True)
...
# ===== صفحة السائق =====
    ...
    if not st.session_state.driver_logged_in:
        st.markdown(f"<h3>{t('driver_login')}</h3>", unsafe_allow_html=True)
        ...
        with col2:
            ...
            st.metric("✅ الحضور المؤكد", coming_today)
        with col3:
                ...
                for _, student in coming_students.iterrows():
                    with st.container():
                        ...
                        <div class='student-card'>
                            ...
                            <div style='color: #10b981;'>✅ {t('status_coming')} - {student['time']}</div>
                        </div>
                        """, unsafe_allow_html=True)
            else:
...
# ===== صفحة الإدارة =====
    ...
    if not st.session_state.admin_logged_in:
        st.markdown(f"<h3>{t('admin_login')}</h3>", unsafe_allow_html=True)
        ...
        with tab1:
            ...
            with st.form("add_student_form"):
                col1, col2 = st.columns(2)
                with col1:
                    new_student_id = st.text_input("رقم الوزارة", placeholder="أدخل رقم الوزارة...")
                    new_student_name = st.text_input("اسم الطالب", placeholder="أدخل اسم الطالب...")
                with col2:
                ...
                new_parent_phone = st.text_input("هاتف ولي الأمر", placeholder="أدخل رقم الهاتف...")
        ...
        with tab3:
            st.header("🔐 إدارة كلمات المرور")
            
            col1, col2 = st.columns(2)
            
            with col1:
                st.subheader("كلمات مرور الباصات")
            ...
            with col2:
                st.subheader("كلمة مرور الإدارة")
                st.text_input("كلمة مرور الإدارة", value=st.session_state.admin_password, type="password", key="admin_pass")
        ...
        with tab4:
            ...
            with col1:
                st.subheader("المظهر")
                theme = st.selectbox("السمة", ["فاتح", "داكن"], index=0 if st.session_state.theme == "light" else 1)
                if theme == "فاتح" and st.session_state.theme != "light":
                    st.session_state.theme = "light"
                    save_data()
                    st.rerun()
                elif theme == "داكن" and st.session_state.theme != "dark":
            ...
            with col2:
                st.subheader("اللغة")
                lang = st.selectbox("اللغة", ["العربية", "English"], index=0 if st.session_state.lang == "ar" else 1)
...
# ===== صفحة الدعم =====
    ...
    tab1, tab2, tab3 = st.tabs(["المساعد الذكي", "التواصل مع المطور", "الأسئلة الشائعة"])
    
    with tab1:
        if len(st.session_state.chat_messages) == 0:
            st.session_state.chat_messages.append({
                "role": "assistant", 
                "content": "مرحباً! أنا المساعد الذكي لنظام الباص. كيف يمكنني مساعدتك؟"
            })
        ai_chatbot()
    ...
    with tab3:
        ...
        with st.expander("كيف أسجل حضور الطالب؟"):
            st.write("""
        ...
        with st.expander("ماذا أفعل إذا نسيت رقم الوزارة؟"):
            st.write("يمكنك التواصل مع إدارة المدرسة للحصول على رقم الوزارة.")
        
        with st.expander("كيف أتابع حالة ابني؟"):
            st.write("""
            1. انتقل إلى صفحة أولياء الأمور
            2. أدخل رقم وزارة الطالب
            3. ستظهر لك حالة الحضور ومعلومات الباص
            """)
...
# ===== صفحة حول النظام =====
    ...
    features = [
        ("🚍", "تسجيل حضور ذكي", "نظام تسجيل حضور آلي وسهل للطلاب"),
        ("📱", "متابعة مباشرة", "متابعة حية لتحركات الباصات والحضور"),
        ("⭐", "تقييم الخدمة", "نظام تقييم متطور لجودة الخدمة"),
        ("🔔", "إشعارات فورية", "إشعارات فورية لأولياء الأمور"),
        ("🎨", "واجهة متطورة", "تصميم حديث وسهل الاستخدام"),
        ("🔒", "أمان وحماية", "نظام حماية متكامل للبيانات")
    ]
...
# ===== الحفظ التلقائي =====
if (datetime.datetime.now() - st.session_state.last_save).seconds > 300:  # كل 5 دقائق
    save_data()
